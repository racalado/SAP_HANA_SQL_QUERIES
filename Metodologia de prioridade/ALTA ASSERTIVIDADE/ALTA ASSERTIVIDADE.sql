--ENERGIA PERDIDA V 11.2 - CRITÉRIO ALTA ASSERTIVIDADE

SELECT NOTA, ANLAGE, ZCGNOMCLI, UTD, DIRECIONADOR, QUEDA_FINAL, ENERGIA_PREV, ENERGIA_PERDIDA, ZCGSITUCC, ZCGTARTYP, GRUPO_CPSP, GERACAO, MUNICIPIO, TEXTONOTA, USERCODE, DATACRIACAO, MED_ANT, MEDIA_12M, QUEDA, ENERGIA_IRR, ENERGIA_DEF, SCORE_TEMPO, SCORE_CRITERIO, SCORE_ENERGIA, PORTECLI, ORDEM FROM 
(
...
--CRITERIO
CASE WHEN (
UPPER(TEXTONOTA) LIKE UPPER('%D1#%') OR
UPPER(TEXTONOTA) LIKE UPPER('%D3#%') OR
UPPER(TEXTONOTA) LIKE UPPER('%D4#%') OR
UPPER(TEXTONOTA) LIKE UPPER('%D5#%') OR
UPPER(TEXTONOTA) LIKE UPPER('%D7#%')
) THEN 0 ELSE 1
...
--ENERGIA_PREV
CASE WHEN ("Energia_def" IS NOT NULL AND "Energia_def" > 0) THEN ROUND("Energia_def",2) ELSE (CASE WHEN (QUEDA IS NULL OR QUEDA = 0) THEN ROUND(MED12M,2) WHEN QUEDA > 0 THEN ROUND((MED_ANT), 2) ELSE 0 END) END
AS ENERGIA_PREV

FROM (SELECT INSTALACAO AS ANLAGE, NOTA, USERCODE, MED_ANT, QUEDA, MED12M, FX1, FX2, "Energia_irr", "Energia_def", DATACRIACAO, ZCGSITUCC, ZCGTARTYP, ZCGNOMCLI, GRUPO_CPSP, CT, GERACAO,  CLI.UTD, CLI.MUNICIPIO, ROUND(DAYS_BETWEEN(DATACRIACAO,CURRENT_DATE)/30,0) AS MESES_NOTA, DAYS_BETWEEN(DATACRIACAO,CURRENT_DATE) AS DIAS_NOTA, TEXTONOTA, DATA_ENCE, CASE WHEN ENE.QUEDA IS NOT NULL THEN ENE.QUEDA WHEN ENE.QUEDA IS NULL AND (ROUND(DAYS_BETWEEN(DATACRIACAO,CURRENT_DATE)/30,0)) IS NOT NULL THEN (ROUND(DAYS_BETWEEN(DATACRIACAO,CURRENT_DATE)/30,0)) END AS QUEDA_FINAL FROM "CLP123432"."PLANILHAO_NOTA_CI" PCI
      LEFT JOIN (SELECT * FROM (SELECT ZCGINSTAL, ZCGACCOUN, ZCGSITUCC, ZCGTARTYP, ZCGNOMCLI, UTD, MUNICIPIO, GRUPO_CPSP, ROW_NUMBER() OVER(PARTITION BY ZCGINSTAL ORDER BY ZCGACCOUN DESC) AS SEQ_INST FROM "CLP123432"."PLANILHAO_CLIENTE") WHERE SEQ_INST = 1 AND ( UPPER("ZCGTARTYP") NOT LIKE  UPPER('%L%'))) CLI ON CLI.ZCGINSTAL = PCI.INSTALACAO 
      LEFT JOIN "CLP123432"."ENERGIA_TOTAL" ENE ON ENE.ANLAGE = PCI.INSTALACAO
      LEFT JOIN (SELECT ZCGINSTALL, MED12M FROM "NEO_PEC"."neo.pec.data::MODEL.TABLES.ZCT_ABT_PROD") ABT ON ZCGINSTALL = PCI.INSTALACAO
      LEFT JOIN "CLP123432"."APOIO_FAIXAS_ENERGIAPERDIDA" FX ON CLI.UTD = DESC_UTD
      LEFT JOIN "CLP123432"."PLANILHAO_INELEGIVEIS" INEL ON INEL.ZCGINSTAL = PCI.INSTALACAO)

WHERE NOTA IS NOT NULL AND DATA_ENCE ='1900-01-01' AND  USERCODE in ('CRIA','DESP','RETI','CRRE','REDI') AND GRUPO_CPSP = 'GRUPO_A' AND ZCGTARTYP NOT LIKE '%L%'  AND CT <> '1PE000' --AND (TEXTONOTA like '%D1#%' OR TEXTONOTA LIKE '%D4%' OR TEXTONOTA LIKE '%D5%')

)
ORDER BY UTD, SCORE_TEMPO, SCORE_CRITERIO, SCORE_ENERGIA, ENERGIA_PREV DESC

) WHERE SCORE_CRITERIO = 0
ORDER BY UTD, ENERGIA_PERDIDA DESC, ENERGIA_PREV DESC