--PLANO DE TRABALHO - 1.	Gestão: Direcionadores Externos por tipo e sub tipo
SELECT SETOR,UTD , 
...
CAST((SUM(CASE WHEN DADOS.CAUSA IS NOT NULL THEN KWH_RECUPERADO ELSE 0 END)/1000) AS DECIMAL(10,2)) AS MWH_REC_TT, 
...
CAST((SUM(CASE WHEN DADOS.CAUSA IS NOT NULL THEN ENERGIA_AGREGADA ELSE 0 END)/1000) AS DECIMAL(10,2)) AS MWH_AGREG_TT
-- TOTAIS - FINAIS

FROM 
(SELECT A.* , B.* FROM
(SELECT  * FROM "CLP123432"."PLANILHAO_NOTA_CI" WHERE LEFT(DATA_ENCE,4)= '2018' AND GRUPO = 'GRUPO_B_SEM_IP' AND tpnota_geracao IS NOT NULL ) AS A

LEFT JOIN 
(SELECT NOTA, ENERGIA_AGREGADA FROM "CLP123432"."zPLANILHAO_NOTA_CI") B
ON A."NOTA" = B.NOTA
